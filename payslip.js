...
// สร้าง HTML โดยตรง ไม่ต้อง puppeteer (ใช้ใน LIFF)
function buildPayslipHtml(d) {
  ...
  // ดึง HTML จาก createFromPayroll แบบ sync โดยสร้าง html ตรงๆ
  // clone logic จาก createFromPayroll แต่ return html เลย
  const payType = d.payType === 'daily' ? 'รายวัน' : 'รายเดือน';
  // เรียก buildHtml function เดียวกับ createFromPayroll
  return _buildPayslipHtml(d);
}

module.exports = { createFromPayroll, buildPayslipHtml, htmlToPdfBuffer, htmlToImageBuffer, htmlToDriveUrl, sendPdfToLine, sendDocToLine, pdfStore, imageStore };
