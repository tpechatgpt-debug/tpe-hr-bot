  ...
  <table>
    <thead>
      <tr>
        ...
        <th style="width:16%">รายละเอียดวันลา</th>
        <th style="width:7%">จำนวน</th>
        <th style="width:17%">รายการเงินหัก</th>
        <th style="width:7%">จำนวน</th>
    ...
    <tbody>
      <tr>
        <td>วันทำงานปกติ</td><td class="c">${fmtN(d.workDays)}</td><td class="n">${fmt(d.basePay)}</td>
        <td>ลาพักร้อน</td><td class="c">${fmtN(d.leaveVac)}</td>
        <td>หักเบิกเงินล่วงหน้า</td><td class="c">${n(d.advance)>0?'1':'0'}</td><td class="n">${zero(d.advance)}</td>
      </tr>
      <tr>
        <td>ทำงานวันหยุด</td><td class="c">${fmtN(d.holidayD)}</td><td class="n">${zero(d.holidayPay)}</td>
        <td>ลากิจ</td><td class="c">${fmtN(d.leaveP)}</td>
        <td>หักประกันสังคม</td><td class="c">${n(d.soc)>0?'1':'0'}</td><td class="n">${fmt(d.soc)}</td>
      </tr>
      <tr>
        <td>ทำงานล่วงเวลา</td><td class="c">${fmtN(d.otH)}</td><td class="n">${zero(d.otPay)}</td>
        <td>ลาป่วย</td><td class="c">${fmtN(d.leaveSick)}</td>
        <td>หัก ณ ที่จ่าย</td><td class="c">${n(d.tax)>0?'1':'0'}</td><td class="n">${zero(d.tax)}</td>
      </tr>
      <tr>
        <td>เบี้ยเลี้ยง</td><td class="c">${n(d.allowance)>0?'1':'0'}</td><td class="n">${zero(d.allowance)}</td>
        <td>ลาไม่รับค่าจ้าง</td><td class="c">${fmtN(d.leaveNoPay)}</td>
        <td>หักขาดงาน</td><td class="c">${n(d.absentDed)>0?'1':'0'}</td><td class="n">${zero(d.absentDed)}</td>
      </tr>
      <tr>
        <td>เบี้ยขยัน</td><td class="c">${n(d.bonus)>0?'1':'0'}</td><td class="n">${zero(d.bonus)}</td>
        <td>ลาคลอด</td><td class="c">${fmtN(d.leaveMat)}</td>
        <td>หักขอลาโดยไม่รับค่าจ้าง</td><td class="c">${n(d.noPayDed)>0?'1':'0'}</td><td class="n">${zero(d.noPayDed)}</td>
      </tr>
      <tr>
        <td>วันหยุดตามประเพณี (รายวัน)</td><td class="c">${fmtN(d.holidayD)}</td><td class="n">${zero(d.holidayPay)}</td>
        <td>ลาหยุดวันเกิด</td><td class="c">${fmtN(d.leaveBday)}</td>
        <td>หักกยศ.</td><td class="c">${n(d.kot)>0?'1':''}</td><td class="n">${n(d.kot)>0?fmt(d.kot):'-'}</td>
      </tr>
      <tr>
        <td>อื่นๆ</td><td></td><td class="n">${zero(d.otherInc)}</td>
        <td colspan="2"></td>
        <td>อื่นๆ</td><td></td><td class="n">${zero(d.otherDed)}</td>
      </tr>
      <tr class="tot-row">
        <td colspan="2" style="text-align:right">รวม รายการเงินได้</td>
        <td class="n">${fmt(totalInc)}</td>
        <td colspan="2"></td>
        <td style="text-align:right">รวม รายการเงินหัก</td>
        <td></td><td class="n">${fmt(totalDed)}</td>
      </tr>
      <tr class="net-row">
        <td colspan="6">เงินเดือนสุทธิ/บาท</td>
...
<div style="margin-top:6px;font-size:9px;color:#aaa;text-align:right">
  สร้างโดยระบบ HR อัตโนมัติ | ${new Date().toLocaleString('th-TH',{timeZone:'Asia/Bangkok'})}
</div>
</body></html>`;

  return await htmlToDriveUrl(html, `PaySlip_${d.name}_${(d.month||'').replace(/ /g,'_')}`);
}
